...
/// `IbcLiteRouterContract` is a wrapper around Addr that provides helpers
/// for working with this contract.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema)]
pub struct IbcLiteRouterContract(pub Addr);

/// `IbcLiteRouterCode` is a wrapper around u64 that provides helpers for
/// initializing this contract.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema)]
pub struct IbcLiteRouterCode(pub u64);
...
impl IbcLiteRouterContract {
    /// Creates a new [`IbcLiteRouterContract`]
    #[must_use]
    ...
    /// `query` creates a new [`IbcLiteRouterContractQuerier`] for this contract.
    #[must_use]
    pub fn query<'a>(&self, querier: &'a QuerierWrapper) -> IbcLiteRouterContractQuerier<'a> {
        IbcLiteRouterContractQuerier::new(querier, self.addr().into_string())
    }
...
impl IbcLiteRouterCode {
    /// `new` creates a new [`IbcLiteRouterCode`]
    #[must_use]
...
impl<'a> IbcLiteRouterContractQuerier<'a> {
    /// Creates a new [`IbcLiteRouterContractQuerier`]
    #[must_use]
    pub const fn new(querier: &'a QuerierWrapper<'a>, addr: String) -> Self {
        Self { querier, addr }
    }

    /// `port_router` sends a [`msg::QueryMsg::PortRouter`] query to this contract.
    ///
    /// # Errors
    /// This function returns an error if the query fails
    pub fn port_router(&self, port_id: impl Into<String>) -> StdResult<String> {
        self.querier.query_wasm_smart(
            &self.addr,
            &msg::QueryMsg::PortRouter {
